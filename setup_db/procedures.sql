...
CREATE PROCEDURE IF NOT EXISTS GetOfficerUnits(IN user_id INT)
BEGIN
SELECT u.ID, u.Name
        FROM Unit u
        JOIN Officer o ON u.CommanderID = o.ID
...
CREATE PROCEDURE IF NOT EXISTS PromoteSoldierToOfficer(
    IN soldierUserID INT,
    IN newRankID INT
)
BEGIN
    DECLARE p_soldierID INT;
    DECLARE p_soldierName VARCHAR(255);
    ...
    -- Получаем ID и имя солдата
    SELECT ID, Name INTO p_soldierID, p_soldierName FROM Soldier WHERE UserID = soldierUserID;

    -- Добавляем солдата в таблицу офицеров
    INSERT INTO Officer (Name, RankID, UserID, Date_of_Commission)
    VALUES (p_soldierName, newRankID, soldierUserID, p_today);

    -- Переносим специальности солдата в таблицу специальностей офицеров
    INSERT INTO OfficerSpecialties (OfficerID, SpecialityID)
    (SELECT (SELECT ID FROM Officer WHERE UserID = soldierUserID), SpecialityID
    FROM SoldierSpecialties
    WHERE SoldierID = p_soldierID);

    -- Обновляем тип пользователя на офицера
    UPDATE User SET UserType = 'officer' WHERE ID = soldierUserID;

    -- Удаляем солдата из таблицы солдат
    DELETE FROM Soldier WHERE ID = p_soldierID;
